...
class Wall:
    ...
    temp_int: float = 20. # Temperature in celsius
    temp_ext: float =  -5.
    ...
    def calc_surface_temperatures(self)-> np.ndarray:
        ...
        temp = self.temp_int - ((self.thermal_resistance_cumsum()*delta_temp)/self.thermal_resistance_tot())
    ...
    def calc_saturation_pressures(self) -> np.ndarray:
        return np.array([610.5*np.exp(17.269*temp/(237.3+temp)) if temp >= 0 else 610.5*np.exp(21.875*temp/(265.5+temp)) for temp in self.calc_surface_temperatures()])
    ...
    def calc_internal_pressures(self) -> np.ndarray:
        ...
        press = p_int - ((self.equivalent_thickness_cumsum()[1:len(self.calc_saturation_pressures()-2)]*delta_p)/self.equivalent_thickness_tot())
    ...
    def calc_profondità_penetrazione(self) -> np.ndarray:
        ...
        return np.sqrt((self.thermal_conductivities()*self.time*3600)/(np.pi*self.densities()*self.specific_heats()))
    ...
    def calc_matrice_trasferimento_layer(self) -> list[np.ndarray]:
        ...
        for i in range(0,len(xi)):
            ...
            z[1][1] = z[0][0] 
            z[0][1] = -(delta[i]/(2*thermal_conductivity[i])) * complex(  (np.sinh(xi[i])*np.cos(xi[i]) + np.cosh(xi[i])*np.sin(xi[i])) , (np.cosh(xi[i])*np.sin(xi[i]) - np.sinh(xi[i])*np.cos(xi[i]))  )
            z[1][0] = -(thermal_conductivity[i]/( delta[i]))  * complex(  (np.sinh(xi[i])*np.cos(xi[i]) - np.cosh(xi[i])*np.sin(xi[i])) , (np.sinh(xi[i])*np.cos(xi[i]) + np.cosh(xi[i])*np.sin(xi[i]))  )
    ...
    def calc_attenuazione(self) -> float:
        "fd"
        return - self.calc_trasmittanza_termica_periodica()/self.thermal_transmittance()

    def calc_phase(self) -> float:
        Zee = self.calc_matrice_trasferimento_tot_ambiente_ambiente()
        # time in hour
        return (np.arctan2(Zee[0][1].imag,Zee[0][1].real))*self.time/(2*np.pi) # tempo in ore
